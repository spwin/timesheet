...
@section('body')
    ...
    <ul class="nav nav-tabs" role="tablist">
        <li role="presentation" class="active"><a href="{{ action('TimesheetController@managersUsers') }}">By user</a></li>
        <li role="presentation"><a href="{{ action('TimesheetController@managersDays') }}">By day</a></li>
    </ul>
    @if($user->days()->where(['submitted' => 1])->leftJoin('week', 'week.id', '=', 'day.week_id')->where('status', '<>', 'none')->where(['week.approved' => 0])->count() > 0)
        <a href="{{ action('TimesheetController@managersUsers') }}" class="inline-block" style="margin-top: 20px;"><i class="fa fa-arrow-circle-o-left"></i> All users</a>
        <h2>
